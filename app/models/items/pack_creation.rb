module PackCreation
  ...
  def add_unlisted_items(character, equipment_choice)
    ...
    when 301 # prayer wheel
      item = create(:character_id => character.id, :item => 206, :description => "Prayer wheel")
  ...
  def create_pack(character, pack)
	...
	###
	def burglars_pack
    item = create(:character_id => character.id, :item => 206, :description => "10 Feet of string")
  ...
  def priests_pack
    item = create(:character_id => character.id, :item => 206, :description => "Alms box")
    item = create(:character_id => character.id, :item => 206, :description => "2 Blocks incense")
    item = create(:character_id => character.id, :item => 206, :description => "Censer")
    item = create(:character_id => character.id, :item => 206, :description => "Vestments")
    [[65, 1], [71, 1], [205, 10], [140, 1], [126, 2], [143, 1]]
  end

  def scholars_pack(character)
    item = create(:character_id => character.id, :item => 72, :description => "Book of lore")
    item = create(:character_id => character.id, :item => 206, :description => "Little bar of sand")
    item = create(:character_id => character.id, :item => 206, :description => "Small knife")
